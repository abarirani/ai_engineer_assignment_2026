...
def flush_all_traces_to_file(output_dir: str = "data/output") -> str:
    """Flush all collected traces from all jobs to a single timestamped file.

    This function aggregates all spans from all job exporters and writes them
    to a single JSON file with a timestamp-based filename.

    Args:
        output_dir: Base output directory (default: "data/output").

    Returns:
        The path to the file where traces were written.
    """
    from datetime import datetime

    # Aggregate all spans from all exporters
    all_traces: list[dict] = []

    for job_id, exporter in _job_exporters.items():
        all_traces.extend(exporter._spans)
